...
def get_rank_data(my_address):
    try:
        rank_url = f'https://trailblazer.mainnet.taiko.xyz/s3/user/rank?address={my_address}'
        headers = {
            'User-Agent': 'Mozilla/5.0 (compatible; TaikoRankChecker/1.0)'
        }
...
def display_taiko_data(my_address):
    ...
    if transaction_data:
        ...
        animated_print("                       Taiko Transaction Data S3             ", color=Fore.GREEN, delay=0.01)
        animated_print("=================================================================", color=Fore.MAGENTA, delay=0.01)
        ...
        if 'adjusted_gas_fee_eth_from_sep16' in transaction_data:
            animated_print(f"Total Gas Fee ETH TAIKO S3   : {transaction_data['adjusted_gas_fee_eth_from_sep16']:.10f} ETH", color=Fore.BLUE, delay=0.01)
        
        if 'total_gas_fee_usd_from_sep16' in transaction_data:
            animated_print(f"Total Gas Fee USD TAIKO S3   : ${transaction_data['total_gas_fee_usd_from_sep16']:.2f}", color=Fore.BLUE, delay=0.01)
        
        if 'tx_count_from_sep16' in transaction_data:
            animated_print(f"Jumlah Tx TAIKO S3           : {transaction_data['tx_count_from_sep16']} transaksi", color=Fore.GREEN, delay=0.01)
    ...
    if rank_data:
        ...
        animated_print("=================================================================", color=Fore.MAGENTA, delay=0.01)
        animated_print("                       Taiko Rank Data S3           ", color=Fore.GREEN, delay=0.02)
        animated_print("=================================================================", color=Fore.MAGENTA, delay=0.01)
